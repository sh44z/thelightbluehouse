  ...
  return (
    <div className='relative'>
            <div className='grid gap-2 grid-cols-[2fr_1fr] overflow-hidden'>
                 <div>
                    {place.addedPhotos?.[0] && (
                    <img onClick={() => setShowAllPhotos(true)} className='aspect-square object-cover cursor-pointer' src={"http://localhost:7000/uploads/"+place.addedPhotos[0]} alt='' /> 
                    )}
                </div>
            <div className='grid'>
                {place.addedPhotos?.[1] && (
                    <img onClick={() => setShowAllPhotos(true)} className='aspect-square object-cover cursor-pointer' src={"http://localhost:7000/uploads/"+place.addedPhotos[1]} alt='' /> 
                )}
                <div className='overflow-hidden'>
                {place.addedPhotos?.[2] && (
                    <img onClick={() => setShowAllPhotos(true)} className='aspect-square object-cover cursor-pointer relative top-2' src={"http://localhost:7000/uploads/"+place.addedPhotos[2]} alt='' /> 
                )}
                </div>
